    #Casos de prueba asociados al usario standar_user
...
Feature: Verificar elementos del menú de navegación
    Como usuario standar_user logueado, quiero navegar por los items que componen el menú de navegación principal.
    ...
    Scenario: Interactuar con el primer item del navbar
        When el usuario se encuentre en cualquier vista del aplicativo
            And procede hacer clic en la opción "<ALL ITEMS>" del navbar
        Then el usuario visualiza la carga de los productos del ecommerce en la interfaz principal "<inventory>"
...
Feature: Validar el filtro de consulta
    Como usuario standar_user logueado, quiero realizar consulta de productos en orden ascendente/descendente
...
Feature: Verificar las caracteriticas de cada tag de productos
    Como usuario standar_user logueado, quiero ver la imagen, nombre, descripción y precio de cada producto
...
Feature: Verificar que los productos se agregen al carrito de compras
    Como usuario standar_user logueado, quiero agregar productos al carrito de compras
...
Feature: Remover los productos agregados al carrito de compras
    Como usuario standar_user logueado, quiero remover los productos agregados al carrito de compras
...
Feature: Verificar cantidades añadidas o removidas de los productos en el carrito de compras
    Como usuario standar_user logueado, quiero agregar y quitar cantidades a los productos agregados al carrito de compras

    @addCantidades
    Scenario: Añañdir y quitar cantidades del producto agregado al carrito de compras
...
Feature: Verificar la opción de continuar la compra agregando más productos al carrito de compras
    Como usuario standar_user logueado, quiero retornar al catálogo de productos desde el carrito de compras
...
Feature: Verificar la compra de los productos
    Como usuario standar_user logueado, quiero realizar la compra de mis productos agergados al carrito
    ...
    Examples:
        ...
        | Products                | cantidad | valor | total |
        | Sauce Labs Bike Light   | 1        | 9.99  |       |
        | Sauce Labs Backpack     | 1        | 29.99 | 55.97 |
    ...
    Scenario: Finalizar la compra de productos
        ...
        Then el usuario visualiza una alerta success de la compra finalizada
            And el usuario detalla que el ícono del carrito de compras queda sin registro de productos
